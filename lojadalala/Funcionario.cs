...
namespace lojadalala
{
  class Funcionario
  {
    ...
    {
      string SQL = "INSERT INTO funcionario (nome, cpf, rg, logradouro, numero, bairro, cidade, estado, cep, datanasc, dataadmi, salario, telefone, celular, idFuncao) VALUES (@nome, @cpf, @rg, @log, @num, @bai, @cid, @est, @cep, @dtn, @dta, @sal, @tel, @cel, @idF)";
      cmd = new MySqlCommand();
      cmd.CommandText = SQL;
    ...
    {
      string SQL = "SELECT id FROM funcionario WHERE nome = '" + nome + "' ORDER BY id DESC LIMIT 1";
    ...
    public void ConsultarMecanico()
    {
      ...
      if (localizado)
      {
        ...
        ds = bd.ConsultarRegistro(SQL, tabela);
        nome = Convert.ToString(ds.Tables["funcionario"].Rows[0]["nome"]);
        cpf = Convert.ToString(ds.Tables["funcionario"].Rows[0]["cpf"]);
        rg = Convert.ToString(ds.Tables["funcionario"].Rows[0]["rg"]);
        logradouro = Convert.ToString(ds.Tables["funcionario"].Rows[0]["logradouro"]);
        bairro = Convert.ToString(ds.Tables["funcionario"].Rows[0]["bairro"]);
        numero = Convert.ToString(ds.Tables["funcionario"].Rows[0]["numero"]);
        cidade = Convert.ToString(ds.Tables["funcionario"].Rows[0]["cidade"]);
        estado = Convert.ToString(ds.Tables["funcionario"].Rows[0]["estado"]);
        cep = Convert.ToString(ds.Tables["funcionario"].Rows[0]["cep"]);
        datanasc = Convert.ToDateTime(ds.Tables["funcionario"].Rows[0]["datanasc"]);
        dataadmi = Convert.ToDateTime(ds.Tables["funcionario"].Rows[0]["dataadmi"]);
        salario = Convert.ToDecimal(ds.Tables["funcionario"].Rows[0]["salario"]);
        telefone = Convert.ToString(ds.Tables["funcionario"].Rows[0]["telefone"]);
        celular = Convert.ToString(ds.Tables["funcionario"].Rows[0]["celular"]);
        idFuncao = Convert.ToInt32(ds.Tables["funcionario"].Rows[0]["idFuncao"]);
      }
      else
      {
    ...
    {
      string SQL = "UPDATE funcionario SET nome = @nome, cpf = @cpf, rg = @rg, logradouro = @log, numero = @num, bairro = @bai, cidade = @cid, estado = @est, cep = @cep, datanasc = @dtn, dataadmi = @dta, salario = @sal, telefone = @tel, celular = @cel, idFuncao = @idF WHERE id = @id";
      cmd = new MySqlCommand();
      cmd.CommandText = SQL;
