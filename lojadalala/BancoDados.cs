...
namespace lojadalala
{
  class BancoDados
  {
    ...
    private string sCon = @"Data Source=localhost; DataBase=lojadalala; User Id=root; Password=jkax10";
    ...
    public void ExcluirRegistro(MySqlCommand cmd)
    {
      ...
      {
          MessageBox.Show("Não foi possível excluir o registro!\n\nErro: " + ex.Message, "Erro de Exclusão", MessageBoxButtons.OK, MessageBoxIcon.Error);
      }
      finally
      {
          //Fecha a conexão
    ...
    public decimal RetornaValor(MySqlCommand cmd)
    {
      decimal valor = 0.00M; 
      if (con.State != ConnectionState.Open)
      {
    ...
    {
      string path = "C:\\LojadaLala\\Backup";
